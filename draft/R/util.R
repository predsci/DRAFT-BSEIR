...
plotDisease <- function(mydata = NULL,  device = 'png') {
    #' Plot The Time Series of the User Provided Incidence
    #'
    #' \code{plotDisease} Uses plotly to plot the user provided disease time series in both png and html formats.
    ...
    #' plotDisease(mydata = mydata,  device = 'pdf')
	...
	pl = ggplot(data = NULL) + scale_x_continuous(name = "Date", limits = c(1, ntps), breaks = 1:ntps, labels = x.axis.label) + 
		scale_y_continuous(name = ylab) + theme(text = element_text(size = 8, color = "gray20", face = "italic"), 
		axis.text.x = element_text(face = "plain", angle = 90, size = 6), axis.text.y = element_text(face = "plain"))
	pl = pl + geom_line(aes(x = 1:ntps, y = mydata$model$raw), col = "darkred", size = 2)	

	pl = pl + annotate("text", x = -Inf, y = Inf, label = title, 
		hjust = 0, vjust = 2.5, col = "black", family = "serif", size = 3.0)
		
	ggsave(file = filename, plot = pl, device = device, width = 14, height = 9, units = "cm")
...
plotResults <- function(rtn = NULL, profile = NULL, tab = NULL, mydata = NULL, device = 'png') {
	...
	plotlist[[1]] = ggplot(data = NULL) + scale_x_continuous(name = "Date", limits = c(1, nperiods), breaks = breaks, labels = labels) + theme(text = element_text(size = 10, color = "gray20", face = "italic"), 
		axis.text.x = element_text(face = "plain", angle = 90), axis.text.y = element_text(face = "plain"))
	...
	plotlist[[1]] = plotlist[[1]] + geom_line(aes(x = data.rnd[, 1], y = data.rnd[, 3], group = data.rnd[, 2]), col = "#E495A5", 
		size = 2, alpha = 0.4) + geom_line(aes(x = 1:nperiodsFit, y = rtn[1:nperiodsFit]), col = "#39BEB1", size = 1) + geom_line(aes(x = 1:nperiodsFit, 
		y = model_mean[1:nperiodsFit]), col = "#099DD7", size = 0.8) + geom_line(aes(x = 1:nperiods, y = obs), col = "black", na.rm = TRUE) + 
		geom_point(aes(x = 1:nperiodsFit, y = obs[1:nperiodsFit]), col = "#24576D", size = 1, na.rm = TRUE)
	...
	if (nperiodsFit < nperiods) {
		plotlist[[1]] = plotlist[[1]] + geom_line(aes(x = (data.rnd.pred[, 1] + nperiodsFit - 1), y = data.rnd.pred[, 3], group = data.rnd.pred[, 
			...
			col = "#099DD7", size = 0.8, linetype = 2) + geom_line(aes(x = nperiodsFit:nperiods, y = rtn[nperiodsFit:nperiods]), 
			col = "#39BEB1", size = 1, linetype = 2) +  geom_line(aes(x = nperiodsFit:nperiods, y = obs[nperiodsFit:nperiods]), 
			col = "black", size = 1, linetype = 2) + geom_rect(aes(xmin = nperiodsFit, xmax = nperiods, 
			ymin = 0, ymax = ymax), fill = "#D497D3", alpha = 0.4)
	}
	
	reg.name = paste0("   ", c("User Data", "Model-Best", "Model-Mean", "Model-Random"))
	plotlist[[1]] = plotlist[[1]] + annotate("text", x = rep(-Inf, 5), y = rep(Inf, 5), label = c(paste0("   ", mydata$FY), reg.name), 
		hjust = rep(0, 5), vjust = seq(from = 2.5, to = 8.5, by = 1.5), col = c("black", "black", "#39BEB1", "#099DD7", "#E495A5"), 
   ...
   if (mydata$epi_model == 3) {
   	...
   	plotlist[[1]] = plotlist[[1]] + geom_line(aes( x = 1:nperiods, y = p.mean * ymax), col = 'darkred', size = 2, alpha = 0.7)
   	plotlist[[1]] = plotlist[[1]] + geom_line(aes( x = 1:nperiods, y = q.mean * ymax), col = 'orange' , size = 2, alpha = 0.7)
   	plotlist[[1]] = plotlist[[1]] + scale_y_continuous(name = ylab, limits = c(0, ymax), sec.axis = sec_axis(~. / ymax, name = "Fractional Contact Rate"))

   } else {
   	
	plotlist[[1]] = plotlist[[1]] +	scale_y_continuous(name = ylab, limits = c(0, ymax)) 
   }
	...
	for (par in hist.params) {
		...
		plotlist[[icount]] = ggplot(data = data, aes(x = x)) + geom_histogram(data = data, aes(y = ..density..), fill = my.color, 
			col = "white", alpha = 0.7) + scale_x_continuous(name = my.par) + scale_y_continuous(name = "") + theme(text = element_text(size = 10, 
			color = "gray20", face = "italic"), axis.text.x = element_text(face = "plain"), axis.text.y = element_text(face = "plain"))
		plotlist[[icount]] = plotlist[[icount]] + annotate("text", x = rep(-Inf, 2), y = rep(Inf, 2), label = c(paste0("   ", "Mean ", 
			my.mean), paste0("   ", "SD ", my.sd)), hjust = rep(0, 2), vjust = seq(from = 2.5, to = 4, by = 1.5), col = c("black", 
			"black"), family = "serif", size = 3.5) + annotate("text", x = rep(Inf, 2), y = rep(Inf, 2), label = my.par, hjust = 2, 
			vjust = 2.5, col = "black", family = "serif", size = 4)

	}

	if (icount == 3) {	
	 mp = grid.arrange(grobs = plotlist, widths = c(1, 1), layout_matrix = rbind(c(1, 1), c(2, 3)))
	} else if (icount == 4) {
	 mp = grid.arrange(grobs = plotlist, widths = c(1, 1, 1), layout_matrix = rbind(c(1, 1, 1), c(2, 3, 4)))	
	} else if (icount == 2) {
	 mp = grid.arrange(grobs = plotlist, widths = c(1, 1), layout_matrix = rbind(c(1, 1), c(2, NA)))	
	} else {
		mp = plotlist[[1]]
	}

	cat("\n\n For a Plot of the Results See: ", filename, "\n\n")	
	ggsave(file = filename, plot = mp, device = device, width = 9, height = 7, units = "in")
...
write.mcmc <- function(tab = NULL, opt.list = NULL, run.list = NULL, mydata = NULL, imask = NULL) {
    ...
    #' @examples
    #' mcmc.single.write{tab = tab, opt.list = opt.list,
    #' run.list = run.list, mydata = mydata, imask = imas}
    ...
    if (!is.null(tab))
        results.list = mcmc(data = tab, start = (ithin * iburn), end = nMCMC, thin = ithin)
